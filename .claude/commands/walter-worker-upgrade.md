...
- refresh coworker
- sync coworker
when-to-use: 'When the user''s walter-worker installation is out of date and needs
...
### Phase 1: Pull latest walter-worker

**MUST** — find the repo, fetch, merge. If conflicts, report and stop.
After this phase, `walter-worker/skills/` is up to date on disk.

Step 1 — Locate the walter-worker repo:

```bash
COWORKER_ROOT=$(git -C ~/project/walter-worker rev-parse --show-toplevel 2>/dev/null || echo "")
if [[ -z "$COWORKER_ROOT" ]]; then
  echo "walter-worker repo not found at ~/project/walter-worker"
  exit 1
fi
```
...
Step 2 — Scan walter-worker skills.
...
for skill_dir in "$COWORKER_ROOT/skills"/*/; do
  ...
  if [[ ! -f "$target" ]]; then
    echo "NEW (walter-worker): $name"
  elif [[ "$skill_file" -nt "$target" ]]; then
    echo "UPDATE (walter-worker): $name"
  else
    echo "SKIP (walter-worker): $name (current)"
  fi
done
```
...
Step 6 — Install project skills to .claude/commands/:

```bash
cd "$COWORKER_ROOT" && python3 -c "
from pathlib import Path
import sys; sys.path.insert(0, 'src')
from coworker.config import install_project_skills
installed = install_project_skills(Path('$COWORKER_ROOT'))
print(f'{installed} new skills installed to .claude/commands/')
"
```
...
Skills installed:
  ...
  ✅ 1 UPDATED
  ⏭  8 skipped (current)
  📋 Project skills synced to .claude/commands/
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  walter-worker-upgrade — Complete
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
