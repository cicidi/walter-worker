...
// ── Cost & Tokens ──
async function loadGraph(){try{const g=await fetchJSON(API+'/graph');const byType=g.stats?.by_type||{};const byFilter=g.stats?.by_filter||{};$('#main').innerHTML='<div class="content"><div class="page-title">🔗 Memory Graph</div><div class="page-subtitle">Knowledge graph — stigmergy-inspired, decay-weighted edges</div><div class="stat-grid mb-lg"><div class="stat-card"><div class="label">Nodes</div><div class="value blue">'+fmtNum(g.stats?.total_nodes||0)+'</div></div><div class="stat-card"><div class="label">Edges</div><div class="value green">'+fmtNum(g.stats?.total_edges||0)+'</div></div>'+Object.entries(byType).map(([k,v])=>'<div class="stat-card"><div class="label">'+escHtml(k)+'</div><div class="value">'+v+'</div></div>').join('')+'</div><div class="grid-2 mb-lg"><div class="panel"><div class="panel-header">Nodes <span class="count">'+(g.nodes||[]).length+'</span></div><div class="panel-body" style="max-height:500px;overflow-y:auto"><table><tr><th>ID</th><th>Type</th><th>Community</th><th>Sessions</th></tr>'+((g.nodes||[]).slice(0,100).map(n=>'<tr><td class="text-xs">'+escHtml(n.label||n.id)+'</td><td><span class="tag tag-skill">'+escHtml(n.type||'?')+'</span></td><td class="text-xs">'+escHtml(n.community||'-')+'</td><td>'+fmtNum(n.session_count||0)+'</td></tr>').join('')||'<tr><td colspan="4" class="text-muted">No nodes</td></tr>')+'</table></div></div><div class="panel"><div class="panel-header">Edges <span class="count">'+(g.edges||[]).length+'</span></div><div class="panel-body" style="max-height:500px;overflow-y:auto"><table><tr><th>Source→Target</th><th>Relation</th><th>Weight</th><th>Filter</th></tr>'+((g.edges||[]).slice(0,100).map(e=>'<tr><td class="text-xs">'+escHtml((e.source||'')+' → '+(e.target||''))+'</td><td><span class="tag tag-tool">'+escHtml(e.relation||'?')+'</span></td><td><span class="'+(e.effective_weight>0.7?'value green':e.effective_weight>0.4?'value yellow':'value red')+'">'+(e.effective_weight||0).toFixed(2)+'</span></td><td><span class="tag '+(e.filter==='strong'?'tag-knowledge':e.filter==='weak'?'tag-skill':'tag-tool')+'">'+escHtml(e.filter||'all')+'</span></td></tr>').join('')||'<tr><td colspan="4" class="text-muted">No edges</td></tr>')+'</table></div></div></div><div class="text-xs text-muted mt-sm">💡 Nodes and edges decay over time when unused. Strong edges (weight ≥0.7) are most reliable.'+(g.nodes&&g.nodes.length===0?'<br>⚠️ <b>Graph is empty.</b> Run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:3px">/ai-coworker-setup-graphify</code> or <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:3px">coworker memory init</code> to populate it.':'')+'</div></div>'}catch(e){$('#main').innerHTML='<div class="content"><div class="page-title">Graph</div><div class="error">Failed: '+e.message+'. Run <code>coworker memory graphify</code> to build the graph.</div></div>'}}
