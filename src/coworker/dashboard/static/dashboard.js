...
async function setSkillStatus(name,status){try{const r=await fetch(API+'/evolution/skills/'+encodeURIComponent(name)+'/status?status='+status,{method:'POST'});const d=await r.json();viewEvolutionSkill(name)}catch(e){alert('Failed: '+e.message)}}
