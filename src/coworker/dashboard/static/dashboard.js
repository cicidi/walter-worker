...
function renderNav() {
  ...
  items.forEach(i => {
    ...
    if (i.toggle) html += ' <span class="subnav-toggle" title="Toggle Hotspots view">↻</span>';
    html += '</div>';
  });
  html += '</div>';
...
async function loadSessionExpand(id,sid){
  ...
  function renderSessionTable(sessions, maxRows, label){
    ...
    slice.forEach(s=>{
      html+=`<tr style="cursor:pointer" onclick="viewSession('${s.id}')"><td class="text-monospace text-xs">${shortId(s.id,20)}</td><td>${ideIconHtml(s.ide)}</td><td class="text-xs">${s.project||'-'}</td><td>${s.feature?`<span class="tag tag-session" style="font-size:9px">${s.feature}</span>`:'-'}</td><td class="text-right">${s.message_count||0}</td><td class="text-right">${s.tool_count||0}</td><td class="text-right">${s.duration_min?s.duration_min+'m':'-'}</td><td class="text-xs text-muted">${fmtTime(s.created_at)}</td></tr>`;
    });
    html+='</table>';
  ...
  // Handle feature expand (prefix "feature:")
  if(sid.startsWith('feature:')){
    const initName=sid.slice(11);
    try{
      const sessions=await fetchJSON(API+'/sessions?limit=2000');
      const filtered=sessions.filter(s=>s.feature===initName);
      c.innerHTML=renderSessionTable(filtered,30);
    }catch(e){c.innerHTML='<div class="text-sm text-muted" style="padding:12px">Error: '+e.message+'</div>';}
    return;
  }
  ...
  try{
    ...
    html+=`<div class="summary-card"><div class="label">Feature</div><div class="text">${s?.feature||'-'}</div></div>`;
...
async function loadOverview(){
  ...
  +'<div class="panel"><div class="panel-header">Recent Sessions <span class="count">latest '+rs.length+'</span></div><div class="panel-body"><table><tr><th>Session</th><th>IDE</th><th>Project</th><th>Feature</th><th class="text-right">Msgs</th><th class="text-right">Tools</th><th class="text-right">Dur</th><th>Started</th><th></th></tr>'
  +rs.map(x=>expRow('<td><span class="clickable" onclick="event.stopPropagation();viewSession(\''+x.id+'\')">'+shortId(x.id,22)+'</span></td><td>'+ideIconHtml(x.ide)+'</td><td>'+(x.project||'-')+'</td><td>'+(x.feature?'<span class="tag tag-session">'+x.feature+'</span>':'-')+'</td><td class="text-right">'+(x.message_count||0)+'</td><td class="text-right">'+(x.tool_count||0)+'</td><td class="text-right">'+((x.duration_min?x.duration_min+'m':(x.created_at&&x.closed_at?Math.max(1,Math.round((new Date(x.closed_at)-new Date(x.created_at))/60000))+'m':'-')))+'</td><td class="text-sm text-muted">'+fmtTime(x.created_at)+'</td>','<div class="flex gap-lg"><div><span class="text-xs text-muted">Duration:</span> <span>'+(x.duration_min||'-')+'m</span></div><div><span class="text-xs text-muted">IDE:</span> '+ideIconHtml(x.ide)+'</div><div><span class="text-xs text-muted">Branch:</span> <span>'+(x.branch||'-')+'</span></div><div><span class="text-xs text-muted">Model:</span> <span>'+(x.model||'-')+'</span></div><div><span class="clickable text-sm" onclick="viewSession(\''+x.id+'\')">View Timeline →</span></div></div>')).join('')
  +'</table></div></div></div>';
}
...
// SESSIONS
let sCache=[],sF={id:'',ide:'',project:'',feature:''};
...
function applySF(){document.querySelectorAll('.sess-filter').forEach(i=>sF[i.dataset.key]=i.value.toLowerCase());
  const f=sCache.filter(x=>(!sF.id||(x.id||'').toLowerCase().includes(sF.id))&&(!sF.ide||(x.ide||'').toLowerCase().includes(sF.ide))&&(!sF.project||(pName(x).toLowerCase().includes(sF.project))&&(!sF.feature||(x.feature||'').toLowerCase().includes(sF.feature))));
  document.getElementById('sBody').innerHTML=f.map(x=>expRow('<td><span class="clickable text-monospace text-xs">'+shortId(x.id,22)+'</span></td><td>'+ideIconHtml(x.ide)+'</td><td>'+pName(x)+(x.project?'':' <span class="text-xs text-muted" style="font-size:9px">from cwd</span>')+'</td><td>'+(x.feature?'<span class="tag tag-session">'+x.feature+'</span>':'-')+'</td><td class="text-right">'+(x.message_count||0)+'</td><td class="text-right">'+(x.tool_count||0)+'</td><td class="text-right">'+((x.duration_min?x.duration_min+'m':(x.created_at&&x.closed_at?Math.max(1,Math.round((new Date(x.closed_at)-new Date(x.created_at))/60000))+'m':'-')))+'</td><td class="text-sm text-muted">'+fmtTime(x.created_at)+'</td>','<div class="loading" style="padding:20px"><div class="spinner"></div><span>Loading...</span></div>',x.id)).join('');}
...
function renderS(d){const tm=d.reduce((a,x)=>a+(x.message_count||0),0),tt=d.reduce((a,x)=>a+(x.tool_count||0),0);
  const allIde=[...new Set(d.map(x=>x.ide).filter(Boolean))].sort(),allProj=[...new Set(d.map(x=>x.project||(x.cwd?x.cwd.split('/').pop():'')).filter(Boolean))].sort(),allInit=[...new Set(d.map(x=>x.feature).filter(Boolean))].sort();
  ...
  +'<span class="search-box" style="flex:1;min-width:120px"><input class="sess-filter" list="sInit" data-key="feature" placeholder="Feature..." oninput="applySF()"/><datalist id="sInit">'+allInit.map(x=>'<option value="'+x+'">').join('')+'</datalist></span>'
  +'</div>'
  +'<div class="panel"><div class="panel-body"><table><tr><th>ID</th><th>IDE</th><th>Project</th><th>Feature</th><th class="text-right">Msgs</th><th class="text-right">Tools</th><th class="text-right">Dur</th><th>Started</th><th></th></tr><tbody id="sBody">'
  +d.map(x=>expRow('<td><span class="clickable text-monospace text-xs" onclick="event.stopPropagation();viewSession(\''+x.id+'\')">'+shortId(x.id,22)+'</span></td><td>'+ideIconHtml(x.ide)+'</td><td>'+pName(x)+(x.project?'':' <span class="text-xs text-muted" style="font-size:9px">via cwd</span>')+'</td><td>'+(x.feature?'<span class="tag tag-session">'+x.feature+'</span>':'-')+'</td><td class="text-right">'+(x.message_count||0)+'</td><td class="text-right">'+(x.tool_count||0)+'</td><td class="text-right">'+((x.duration_min?x.duration_min+'m':(x.created_at&&x.closed_at?Math.max(1,Math.round((new Date(x.closed_at)-new Date(x.created_at))/60000))+'m':'-')))+'</td><td class="text-sm text-muted">'+fmtTime(x.created_at)+'</td>','<div class="loading" style="padding:20px"><div class="spinner"></div><span>Loading...</span></div>',x.id)).join('')
...
function renderFileView(data) {
  ...
  data.forEach(x => {
    ...
    if (x.project) f.projs.add(x.project); if (x.feature) f.inits.add(x.feature);
  ...
  $('#main').innerHTML = '<div class="content"><div class="page-title">Files</div><div class="page-subtitle">Files read/written — click for history</div>'
    ...
    + '<th style="width:14%;cursor:col-resize" onmousedown="cRes(event,4)">Project</th><th style="width:14%;cursor:col-resize" onmousedown="cRes(event,5)">Feature</th>'
  ...
  try {
    ...
    c.innerHTML = '<div class="text-xs text-muted mb-sm">' + data.length + ' ops</div>'
      + data.slice(0, 50).map(x => '<div class="flex gap-md" style="padding:3px 0;font-size:11px;border-bottom:1px solid rgba(30,34,48,0.3)">'
        ...
        + '<span class="text-xs text-muted">' + (x.feature || '') + '</span>'
...
async function _loadKnowledgeSessions(id, kid){
  ...
  try{
    ...
    if(filtered.length>0){
      html+='<table style="width:100%"><tr><th>Session</th><th>IDE</th><th>Project</th><th>Feature</th><th>Started</th></tr>';
      filtered.forEach(s=>{
        html+=`<tr style="cursor:pointer" onclick="viewSession('${s.id}')"><td class="text-monospace text-xs">${shortId(s.id,20)}</td><td>${ideIconHtml(s.ide)}</td><td class="text-xs">${s.project||'-'}</td><td>${s.feature?`<span class="tag tag-session" style="font-size:9px">${s.feature}</span>`:'-'}</td><td class="text-xs text-muted">${fmtTime(s.created_at)}</td></tr>`;
      });
      html+='</table>';
    }else{
      kSess.forEach(s=>{
...
// FEATURES
async function loadFeatures(){const d=await fetchJSON(API+'/features');
  $('#main').innerHTML='<div class="content"><div class="page-title">Features</div><div class="page-subtitle">Cross-session workstreams</div>'
  +(d.length===0?'<div class="panel"><div class="empty-state"><div class="icon">📋</div>No features</div></div>':
  '<div class="stat-grid mb-lg"><div class="stat-card"><div class="label">Features</div><div class="value yellow">'+d.length+'</div></div><div class="stat-card"><div class="label">Sessions</div><div class="value blue">'+d.reduce((a,x)=>a+x.session_count,0)+'</div></div><div class="stat-card"><div class="label">Tools</div><div class="value green">'+fmtNum(d.reduce((a,x)=>a+x.tool_count,0))+'</div></div></div>'
  +'<div class="panel"><div class="panel-body"><table><tr><th>Feature</th><th>Project</th><th class="text-right">Sessions</th><th class="text-right">Tools</th><th></th></tr>'
  +d.map(x=>expRow('<td><span class="tag tag-session">'+x.feature+'</span></td><td>'+(x.project||'-')+'</td><td class="text-right">'+x.session_count+'</td><td class="text-right">'+x.tool_count+'</td>','<div class="loading" style="padding:20px"><div class="spinner"></div><span>Loading sessions...</span></div>','feature:'+x.feature)).join('')
  +'</table></div></div>')+'</div>';
}
...
async function viewSession(id){const[det,time]=await Promise.all([fetchJSON(API+'/sessions/'+id),fetchJSON(API+'/sessions/'+id+'/timeline').catch(()=>[])]);const s=det.session,sum=det.summary,st=det.stats||{};
  ...
  $('#main').innerHTML='<div class="content"><div class="flex flex-between mb-lg"><div><div class="page-title">Session Detail</div><div class="page-subtitle">'+ideIconHtml(s.ide)+''+(s.project?' <span class="tag tag-file">'+s.project+'</span>':'')+(s.feature?' <span class="tag tag-session">'+s.feature+'</span>':'')+' <span class="text-muted" style="margin-left:8px;font-size:11px">'+shortId(s.id,30)+'</span></div></div><span class="clickable" onclick="goBack()">← Back</span></div>'
...
async function loadSessionLog(sid){
  ...
  try{
    ...
    el.innerHTML=entries.map(e=>e.type==='msg'
      ?'<div style="padding:1px 0;color:'+(e.role==='user'?'var(--accent)':'var(--green)')+'">'+(e.role==='user'?'👤':'🤖')+' <span style="color:var(--text-muted);font-size:10px">'+(e.ts?new Date(e.ts).toLocaleTimeString():'')+'</span> '+escHtml((e.content||'').slice(0,400))+'</div>'
      :'<div style="padding:1px 0"><span style="color:var(--yellow)">⚡</span> <span style="color:var(--text-muted);font-size:10px">'+(e.ts?new Date(e.ts).toLocaleTimeString():'')+'</span> <b style="color:var(--text-primary)">'+escHtml(e.tool||'?')+'</b> <span style="color:var(--text-secondary)">'+escHtml((e.detail||'').slice(0,200))+'</span></div>'
    ).join('');
...
const INFO_MAP={summary:{'Sessions':{source:'overview.total_sessions',purpose:'All AI coding sessions ever recorded'},'Messages':{source:'overview.total_messages',purpose:'User + assistant messages across all sessions'},'Tool Calls':{source:'overview.tool_distribution',purpose:'Total tool invocations (Read, Bash, Edit, etc.)'},'Skills':{source:'overview.total_skills',purpose:'Unique slash commands and skills invoked'},'Knowledge':{source:'overview.total_knowledge',purpose:'Cross-session validated insights and patterns'}},sessions:{'Sessions':{source:'sessions DB table',purpose:'Filtered list of all recorded sessions'},'Messages':{source:'SUM of message_count',purpose:'Total messages across filtered sessions'},'Tools':{source:'SUM of tool_count',purpose:'Total tool calls across filtered sessions'}},projects:{'Projects':{source:'api/projects',purpose:'Number of distinct projects with sessions'},'Sessions':{source:'SUM of session_count',purpose:'Total sessions across all projects'},'Tools':{source:'SUM of total_tools',purpose:'Total tool calls across all projects'}},health:{'Models':{source:'api/models',purpose:'Distinct AI models used across sessions'},'Total Cost':{source:'api/models total_cost SUM',purpose:'Estimated API cost across all models'},'Tokens':{source:'api/models total_tokens SUM',purpose:'Total input + output tokens consumed'},'Avg Latency':{source:'api/models avg_duration_ms AVG',purpose:'Average tool call duration in milliseconds'}},errors:{'Tool Errors':{source:'api/errors tool_errors',purpose:'Tools that returned errors, grouped by tool name'},'Error-Prone Sessions':{source:'api/errors session_errors',purpose:'Sessions with the most tool errors'}},cost:{'Model':{source:'api/cost-analytics model_stats',purpose:'Per-model cost and token breakdown'}},quality:{'Coverage Metrics':{source:'api/data-quality',purpose:'Percentage of sessions with complete metadata'}}};
function getInfo(v,l){const m=INFO_MAP[v];if(m&&m[l])return m[l];return{title:l,source:'/'+v,purpose:'Shows '+l.toLowerCase()+' data for the '+v+' view'};}
...
// ── Evolution (self-evolving-agent feature) ──
...
async function memAction(a){try{const r=await fetch(API+'/memory/'+a,{method:'POST',headers:{'X-Coworker-Dashboard':'1'}});const d=await r.json();alert(a+': '+d.status);loadMemory()}catch(e){alert('Failed: '+e.message)}}

// ── Cost & Tokens ──
async function loadGraph(){try{const g=await fetchJSON(API+'/graph');const byType=g.stats?.by_type||{};const byFilter=g.stats?.by_filter||{};$('#main').innerHTML='<div class="content"><div class="page-title">🔗 Memory Graph</div><div class="page-subtitle">Knowledge graph — stigmergy-inspired, decay-weighted edges</div><div class="stat-grid mb-lg"><div class="stat-card"><div class="label">Nodes</div><div class="value blue">'+fmtNum(g.stats?.total_nodes||0)+'</div></div><div class="stat-card"><div class="label">Edges</div><div class="value green">'+fmtNum(g.stats?.total_edges||0)+'</div></div>'+Object.entries(byType).map(([k,v])=>'<div class="stat-card"><div class="label">'+escHtml(k)+'</div><div class="value">'+v+'</div></div>').join('')+'</div><div class="panel mb-lg" style="height:70vh"><div class="panel-header">Interactive Graph <span class="count">'+(g.nodes||[]).length+' nodes</span></div><iframe src="/graph.html" style="width:100%;height:calc(100% - 36px);border:none;border-radius:0 0 8px 8px"></iframe></div><div class="text-xs text-muted mt-sm">💡 Nodes and edges decay over time when unused. Strong edges (weight ≥0.7) are most reliable.'+(g.nodes&&g.nodes.length===0?'<br>⚠️ <b>Graph is empty.</b> Run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:3px">/walter-worker-setup-graphify</code> or <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:3px">coworker memory init</code> to populate it.':'')+'</div></div>'}catch(e){$('#main').innerHTML='<div class="content"><div class="page-title">Graph</div><div class="error">Failed: '+e.message+'. Run <code>coworker memory graphify</code> to build the graph.</div></div>'}}
