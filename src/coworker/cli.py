...
@click.option("--phases", default="all", help="Comma-separated phases: prd,spec,web,code,all")
...
def find_issues_run(project, phases, output):
    ...
    out_path = output or f"docs/self-evolving-agent/state/issues-found-{today}-auto.md"
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)

    console.print(f"[bold]🔍 Find-Issues — 甲方质检[/bold]")
    ...
    if "all" in phases_list or "prd" in phases_list:
        prd_path = f"docs/self-evolving-agent/prd/self-evolving-agent-prd.md"
        if os.path.exists(prd_path):
            lines = open(prd_path).readlines()
            reqs = [l for l in lines if l.strip().startswith("- R") or "R1" in l or "R2" in l]
            findings.append(f"## PRD Scan: {len(reqs)} requirement references found in {prd_path}")
        else:
            findings.append(f"## PRD Scan: {prd_path} not found")

    if "all" in phases_list or "spec" in phases_list:
        spec_path = f"docs/self-evolving-agent/spec/self-evolving-agent-spec.md"
        if os.path.exists(spec_path):
            sections = [l for l in open(spec_path).readlines() if l.startswith("## §")]
            findings.append(f"## Spec Scan: {len(sections)} sections in {spec_path}")
        else:
            findings.append(f"## Spec Scan: {spec_path} not found")

    if "all" in phases_list or "web" in phases_list:
        findings.append("## Web Research: Use WebSearch tool interactively for best practices")
        findings.append("  (WebSearch requires interactive Claude session)")

    if "all" in phases_list or "code" in phases_list:
        # Quick code audit
        import subprocess
        r = subprocess.run(["python3", "-m", "pytest", "tests/python/", "-q", "--tb=no"],
                          capture_output=True, text=True, timeout=120)
        test_status = "PASS" if r.returncode == 0 else "FAIL"
        findings.append(f"## Code Audit: Tests {test_status}")
        # Check git status
        r = subprocess.run(["git", "status", "--short"], capture_output=True, text=True)
        mods = [l for l in r.stdout.strip().split("\n") if l.strip() and not l.startswith("??")]
