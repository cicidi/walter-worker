...
|-----------|----------------|
| **CLI Engine** | Parse `ai-coworker <command> [args]`; dispatch to skill manager or dashboard server; manage lifecycle (init, self-update). |
...
All commands are invoked as `ai-coworker <command> [subcommand] [options]`.
...
CREATE TABLE session_summaries (
    ...
    generated_by TEXT DEFAULT 'ai-coworker',
    created_at TEXT DEFAULT (datetime('now'))
);
```
...
```markdown
# Global CLAUDE.md – ai-coworker User Global Preferences
- User: <name>
...
- Context: Current task (injected by ai-coworker from memory)
...
5. **Manual override**: Users can `ai-coworker knowledge block <id>` to suppress a specific memory entry.
...
| **OpenCode / Continue** | Separate MCP block in config | `ai-coworker mcp install` adds profile for model loader. |
