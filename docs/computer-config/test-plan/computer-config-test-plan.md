...
| 2026-08-01 | 0.3.0 | Post GLM-5.2 review (scope-corrected): TU-5 → port-as-is (statusline unchanged, model field untouched); add TI-4b (legacy grey-theme migration), TR-6 (walter-worker manifest exclusion); TI-7 → rm -rf + statusLine pop; TR-2 → preserve rich version as optional; fix 0/1/2 → 0/1/2/3 in §6 |
...
Covers two workstreams: the claude-tmux-config project (A) and the walter-worker
strip (B).
...
3. **Regression tests** — walter-worker existing test suite stays green
...
| TI-4b | install component 2 (y, legacy `# walter-worker status bar` present) | prompts "Replace grey theme? [y/N]"; on `y` old block stripped + full BB theme deployed; on `n` only status_info.sh deployed |
...
### 3.3 walter-worker strip regression tests

| ID | Case | Expected |
|----|------|----------|
| TR-1 | install.sh has no tmux reference | `grep tmux setup/install.sh` shows no deploy logic |
| TR-2 | setup/status_info.sh removed from walter-worker | file gone from walter-worker; rich version preserved as `claude-tmux-config/assets/status_info-rich.sh` (optional) |
...
| TR-6 | walter-worker manifest excludes statusline | `~/.coworker/install-manifest.json` has NO entries under `~/.claude/statusline/` after coworker install |
...
# walter-worker full suite
cd ~/project/walter-worker && python -m pytest
# bats install tests
cd ~/project/walter-worker && bats tests/setup/*.bats
```
...
| FR-10/11 walter-worker strip | TR-1/2/3/4/5 |
| FR-12 legacy grey-theme migration | TI-4b |
| FR-13 walter-worker manifest exclusion | TR-6 |
...
5. Confirm walter-worker install no longer touches tmux
6. (Existing walter-worker user) confirm grey-theme → Benjamin Blue migration prompt appears
...
| L4 | walter-worker regression | `pytest` + `bats` | full suite green |
...
4. walter-worker `pytest` + `bats` → green, and `install-manifest.json` contains no
