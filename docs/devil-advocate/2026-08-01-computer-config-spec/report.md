...
| C4 | **Manifest format unspecified** (word 8×, zero schema); cross-project conflict: ai-coworker install.sh recursively walks `~/.claude/`, would claim claude-tmux-config files | Spec 6.3/6.4; ai-coworker install.sh Step 16-second walks `~/.claude/` | Confirmed |
...
| U2 | Cross-project manifest conflict | ai-coworker uninstall could delete claude-tmux-config deployed assets in `~/.claude/statusline/` |
...
2. **Define atomic write mechanism**: specify temp-file + `fsync` + `os.rename` in the spec; reference ai-coworker's existing `_write_json_atomic` in `src/coworker/adapters/claude.py` (it already implements this correctly — cite it).
...
2. **Cross-project manifest conflict (C4/U2)** — data-loss risk: ai-coworker uninstall deletes claude-tmux-config files
