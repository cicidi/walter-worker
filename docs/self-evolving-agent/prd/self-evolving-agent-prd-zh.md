...
> **什么被自动更新，什么不被更新：** "更新上下文"和"记录"步骤仅修改 **CLAUDE.local.md**（个人工作上下文，不提交到 git）、`~/.coworker/skills/` 中的技能和 `~/.coworker/memory/` 中的记忆。**CLAUDE.md**（团队共享文件，提交到 git）永远不会被自动修改——第 5.1/5.3 节的自我进化规则由人工一次性写入，而非 agent 自动修改。这种分离确保 agent 仅进化个人上下文，不会改变团队约定。
